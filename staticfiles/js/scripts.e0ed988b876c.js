...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            fetch(`/profile/content/${type}/`)
                .then(response => response.text())
                .then(data => {
                    contentContainer.innerHTML = data; // Обновляем содержимое
                })
...
document.addEventListener('wheel', function(e) {
    if (e.deltaY > 0) {
        window.location.href = document.querySelector('.next-video').getAttribute('href');
    } else if (e.deltaY < 0) {
        window.location.href = document.querySelector('.prev-video').getAttribute('href');
    }
});
